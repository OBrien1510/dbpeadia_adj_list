...
class MultiLayerPerceptron:
    ...
    def __init__(self, layers=(2, 2, 1, ), hidden_activation="sigmoid", output_activation="linear", linear_factor=0.05, max_iters=20000, learning_rate=0.5, verbose=(True, 10,), weight_update=10, loss="mse", train=True, output_file=None):
        ...
        """
        Layers: A tuple containing the number of neurons at each level from input to output
        hidden_activation: The activation function to be used in all hidden layers
        output_activation: The activation function to be use at the output layer
        linear_factor: (Optional) For when linear or relu activation functions are used.
                sometimes outputs of these functions need to be moderated
        max_iters: The specified number of epochs to train for
        learning_rate: The rate at which weights will be updated
        verbose: (Optional) tuple - set first element to True for information of loss function
            set the second element to specify how often to be notified of such records
            according to the number of epochs past (eg every 10 epochs)
        weight_update: How often to update the weights (eg every 10 epochs)
        loss: Either 'mse' for mean squared error or 'crossentropy' for cross entropy
        train: (Optional) Boolean - Set to False if you desire raw prediction outputs
            eg direct outputs for a softmax function opposed to one specified class for example
        output_file: (Optional) Used to write the results of each epoch to a text file at the
                specified file path
        """
    ...
    def relu(self, x):
       ...
       return np.where(x > 0, x, x*self.linear_factor)

    def relu_d(self,x ):

        return np.where(x > 0, x, self.linear_factor)
